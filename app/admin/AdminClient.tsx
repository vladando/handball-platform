...
export default function AdminClient({ clubs, players, interactions, users, stats }: any) {
  ...
  const pendingPayment = clubs.filter((c: any) => c.subscriptionStatus === "PENDING_PAYMENT");
  ...
  async function activateSubscription(clubId: string, action: "ACTIVATE" | "RESET") {
    await fetch("/api/admin/clubs/subscription", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
          ...
          <div className="tab-content">
            <div className="grid-4" style={{ marginBottom: 32 }}>
              {[
                ...
                { label: "Pending Payments",     val: pendingPayment.length, alert: pendingPayment.length > 0 },
              ].map(s => (
          ...
          <div className="tab-content">
            {pendingPayment.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                <div style={{ padding: "12px 18px", background: "rgba(232,255,71,0.05)", border: "1px solid rgba(232,255,71,0.2)", borderRadius: "var(--radius)", fontSize: "0.85rem", color: "var(--muted)" }}>
                  ⚠ These clubs have confirmed payment intent. Activate their subscription once you confirm the bank transfer (€1,000/year).
                </div>
                {pendingPayment.map((club: any) => (
                  <div key={club.id} className="card">
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
                      <div>
                        ...
                        <div style={{ fontSize: "0.82rem", color: "var(--muted)", marginTop: 4 }}>
                          {club.user?.email} · {club.city}, {club.country}
                        </div>
                      </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16, padding: "14px 16px", background: "var(--card2)", borderRadius: "var(--radius)", border: "1px solid var(--border)", fontSize: "0.85rem" }}>
                      {[
                        ...
                        { label: "Contact Phone", val: club.contactPhone || "—" },
                        { label: "League", val: club.leagueName || "—" },
                        { label: "Verified", val: club.verifiedAt ? new Date(club.verifiedAt).toLocaleDateString() : "—" },
                      ].map(row => (
                        <div key={row.label}>
                          <div style={{ fontSize: "0.68rem", color: "var(--muted)", textTransform: "uppercase", marginBottom: 2 }}>{row.label}</div>
                          <div style={{ color: "var(--white)" }}>{row.val}</div>
                        </div>
                      ))}
                    </div>
                    <div style={{ display: "flex", gap: 12 }}>
                      <button className="btn btn-primary" style={{ minWidth: 200, justifyContent: "center" }} onClick={() => activateSubscription(club.id, "ACTIVATE")}>
                        ✓ Activate Subscription (€1,000/yr)
                      </button>
                      <button className="btn btn-outline" style={{ justifyContent: "center" }} onClick={() => activateSubscription(club.id, "RESET")}>
                        ✕ Decline
                      </button>
                    </div>
                  </div>
            ...
            {/* All clubs subscription overview */}
            <div style={{ marginTop: 40 }}>
              <h4 style={{ textTransform: "uppercase", marginBottom: 16 }}>All Clubs — Subscription Status</h4>
              <div className="card" style={{ padding: 0, overflow: "hidden" }}>
                <div className="table-wrap"><table className="table">
                  <thead><tr><th>Club</th><th>Email</th><th>Sub Status</th><th>Valid Until</th><th>Action</th></tr></thead>
                  <tbody>
                    {clubs.map((club: any) => (
                      <tr key={club.id}>
                        <td style={{ fontFamily: "var(--font-display)", fontWeight: 700, textTransform: "uppercase" }}>{club.name}</td>
                        <td style={{ fontFamily: "var(--font-mono)", fontSize: "0.78rem", color: "var(--muted)" }}>{club.user?.email}</td>
                        <td>
                          <span className={`badge ${club.subscriptionStatus === "ACTIVE" ? "badge-green" : club.subscriptionStatus === "PENDING_PAYMENT" ? "badge-accent" : "badge-muted"}`}>
                            {club.subscriptionStatus}
                          </span>
                        </td>
                        <td style={{ fontFamily: "var(--font-mono)", fontSize: "0.78rem", color: "var(--muted)" }}>
                          {club.subscriptionEndsAt ? new Date(club.subscriptionEndsAt).toLocaleDateString() : "—"}
                        </td>
                        <td>
                          {club.subscriptionStatus !== "ACTIVE" && club.verificationStatus === "VERIFIED" ? (
                            <button className="btn btn-outline" style={{ fontSize: "0.72rem", padding: "4px 10px" }} onClick={() => activateSubscription(club.id, "ACTIVATE")}>
                              Activate
                            </button>
                          ) : club.subscriptionStatus === "ACTIVE" ? (
                            <button className="btn btn-danger" style={{ fontSize: "0.72rem", padding: "4px 10px" }} onClick={() => activateSubscription(club.id, "RESET")}>
                              Deactivate
                            </button>
            ...
            <div className="modal" style={{ maxWidth: 680, width: "100%", maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: "0.75rem", color: "var(--muted)", textTransform: "uppercase", marginBottom: 10, letterSpacing: "0.08em" }}>
                  Search History ({clubModal.searchLogs?.length ?? 0} searches)
                </div>
                {(!clubModal.searchLogs || clubModal.searchLogs.length === 0) ? (
                  <div style={{ padding: "14px 16px", background: "var(--card2)", borderRadius: 8, fontSize: "0.8rem", color: "var(--muted)", textAlign: "center", border: "1px dashed var(--border)" }}>
                    No searches recorded yet
                  </div>
                ) : (
                  <div style={{ maxHeight: 240, overflowY: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
                    {clubModal.searchLogs.slice().reverse().map((log: any, i: number) => (
                      <div key={i} style={{ display: "flex", gap: 10, flexWrap: "wrap", padding: "7px 12px", background: "var(--card2)", borderRadius: 6, fontSize: "0.78rem", color: "rgba(245,243,238,0.85)", border: "1px solid var(--border)", alignItems: "center" }}>
                        <span style={{ color: "var(--muted)", fontFamily: "var(--font-mono)", fontSize: "0.72rem" }}>
                          {new Date(log.searchedAt).toLocaleDateString()} {new Date(log.searchedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                        </span>
                        {log.position ? <span style={{ color: "var(--accent)" }}>📍 {log.position.replace(/_/g, " ")}</span> : <span style={{ color: "var(--muted)" }}>All Positions</span>}
                        {log.nationality ? <span>🌍 {log.nationality}</span> : <span style={{ color: "var(--muted)" }}>All Nations</span>}
                        {(log.minHeight || log.maxHeight) && <span>📏 {log.minHeight ?? 160}–{log.maxHeight ?? 220}cm</span>}
                        {(log.minAge || log.maxAge) && <span>🎂 {log.minAge}–{log.maxAge ?? "∞"}y</span>}
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div style={{ marginBottom: 20, padding: "14px 16px", background: "var(--card2)", borderRadius: "var(--radius)", border: "1px solid var(--border)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                  <div>
                    <div style={{ fontSize: "0.68rem", color: "var(--muted)", textTransform: "uppercase", marginBottom: 4 }}>Subscription</div>
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <span className={`badge ${clubModal.subscriptionStatus === "ACTIVE" ? "badge-green" : clubModal.subscriptionStatus === "PENDING_PAYMENT" ? "badge-accent" : "badge-muted"}`}>
                        {clubModal.subscriptionStatus}
                      </span>
                      {clubModal.subscriptionEndsAt && (
                        <span style={{ fontSize: "0.78rem", color: "var(--muted)" }}>
                          until {new Date(clubModal.subscriptionEndsAt).toLocaleDateString()}
                        </span>
                      )}
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    {clubModal.subscriptionStatus !== "ACTIVE" && clubModal.verificationStatus === "VERIFIED" && (
                      <button className="btn btn-primary" style={{ fontSize: "0.78rem", padding: "6px 14px" }}
                        onClick={() => { activateSubscription(clubModal.id, "ACTIVATE"); setClubModal(null); }}>
                        ✓ Activate Subscription
                      </button>
                    )}
                    {clubModal.subscriptionStatus === "ACTIVE" && (
                      <button className="btn btn-danger" style={{ fontSize: "0.78rem", padding: "6px 14px" }}
                        onClick={() => { activateSubscription(clubModal.id, "RESET"); setClubModal(null); }}>
                        Deactivate
                      </button>
                    )}
                  </div>
