...
export default function PlayerProfileClient({ player }: { player: any }) {
  ...
  const tabs = [{ id:"bio", label:"Bio" },{ id:"career", label:"Career" },{ id:"videos", label:`Videos (${player.videos?.length??0})` },{ id:"medical", label:"Medical" }];
  ...
  return (
    <div>
      ...
      {tab === "bio" && (
        <div>{player.bio ? <div style={{ fontSize:"1rem", lineHeight:1.8, color:"rgba(245,243,238,0.85)", maxWidth:640 }}>{player.bio}</div> : <div style={{ color:"var(--muted)", fontStyle:"italic" }}>No bio provided yet.</div>}</div>
      )}

      {tab === "career" && (
        <div>
          {!player.careerEntries?.length ? <div style={{ color:"var(--muted)" }}>No career history added yet.</div> : (
            <div className="timeline">
              {player.careerEntries.map((e: any) => (
                <div key={e.id} className={`timeline-item${e.isCurrentClub?" current":""}`}>
                  <div style={{ display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
                    <div>
                      <div style={{ fontFamily:"var(--font-display)", fontWeight:700, fontSize:"1.15rem", textTransform:"uppercase" }}>{e.clubName}{e.isCurrentClub && <span className="badge badge-green" style={{ marginLeft:8 }}>Current</span>}</div>
                      <div style={{ fontSize:"0.85rem", color:"var(--muted)" }}>{e.country}</div>
                    </div>
                    <div style={{ fontFamily:"var(--font-mono)", fontSize:"0.8rem", color:"var(--muted)" }}>{new Date(e.startDate).getFullYear()} — {e.endDate ? new Date(e.endDate).getFullYear() : "Present"}</div>
                  </div>
      ...
      {tab === "videos" && (
        <div>
          {!player.videos?.length ? <div style={{ color:"var(--muted)" }}>No videos uploaded yet.</div> : (
      ...
      {tab === "medical" && (
        <div>
          {!player.medicalRecords?.length ? <div style={{ color:"var(--muted)" }}>No public medical records.</div> : (
              ...
              {player.medicalRecords.map((r: any) => (
                <div key={r.id} className="card" style={{ padding:"20px 24px" }}>
                  <div style={{ marginBottom:8 }}>
                    ...
                    <span style={{ fontFamily:"var(--font-display)", fontWeight:700, textTransform:"uppercase", fontSize:"1rem" }}>{r.testName??r.injuryType??"Record"}</span>
                  </div>
                  {r.testResult && <div style={{ fontFamily:"var(--font-display)", fontWeight:700, fontSize:"1.6rem", color:"var(--accent)" }}>{r.testResult} {r.testUnit && <span style={{ fontSize:"0.8rem", color:"var(--muted)" }}>{r.testUnit}</span>}</div>}
