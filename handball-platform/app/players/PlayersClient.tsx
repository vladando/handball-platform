...
export default function PlayersClient({ players, positions, posLabels, nationalities }: any) {
  ...
  const filtered = useMemo(() => players.filter((p: any) => {
    ...
    if (q) { const s = `${p.firstName} ${p.lastName} ${p.currentClub??""} `.toLowerCase(); if (!s.includes(q.toLowerCase())) return false; }
    return true;
    ...
    <>
      <div className="filter-sidebar">
        <div className="filter-title">
          Filters
          <button onClick={() => { setQ(""); setPos(""); setNat(""); setMinH(160); setMaxH(220); }} style={{ fontSize:"0.75rem", fontFamily:"var(--font-mono)", color:"var(--muted)", background:"none", border:"none", cursor:"pointer" }}>Reset</button>
        </div>
        ...
        <div style={{ fontSize:"0.8rem", color:"var(--muted)", textAlign:"center" }}>{filtered.length} player{filtered.length !== 1 ? "s" : ""} found</div>
      </div>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
          ...
          <div style={{ display:"flex", gap:4 }}>
            {(["grid","list"] as const).map(v => (
              <button key={v} onClick={() => setView(v)} style={{ background: view===v?"var(--accent)":"transparent", color: view===v?"var(--black)":"var(--muted)", border:"1px solid var(--border)", borderRadius:"var(--radius)", padding:"6px 12px", cursor:"pointer", fontSize:"0.8rem" }}>
                {v === "grid" ? "⊞" : "☰"}
              </button>
            ))}
          </div>
        ...
        {filtered.length === 0 ? (
            ...
            {filtered.map((p: any) => (
              <Link key={p.id} href={`/players/${p.slug}`} className="player-card">
                <div className="player-card-photo">
                  {p.photoUrl ? <img src={p.photoUrl} alt={p.firstName} style={{ width:"100%", height:"100%", objectFit:"cover" }} /> : <span style={{ fontSize:"3rem", zIndex:1 }}>🏐</span>}
                  <div style={{ position:"absolute", top:10, left:10, zIndex:2 }}><span className="pos-pill">{POS_SHORT[p.position]}</span></div>
                  {p.isAvailable && <div style={{ position:"absolute", top:10, right:10, zIndex:2 }}><span className="badge badge-green">● Available</span></div>}
                </div>
          ...
          <div className="card" style={{ padding:0, overflow:"hidden" }}>
            <table className="table">
              <thead><tr><th>Player</th><th>Pos</th><th>Nation</th><th>Height</th><th>Age</th><th>Salary</th><th></th></tr></thead>
              <tbody>
                {filtered.map((p: any) => (
                  <tr key={p.id}>
                    <td>
