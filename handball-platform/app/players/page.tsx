...
export default async function PlayersPage() {
  const players = await prisma.player.findMany({ where:{ isAvailable:true }, orderBy:{ createdAt:"desc" }, take:100 }).catch(() => []);
  const nationalities = await prisma.player.findMany({ select:{ nationality:true }, distinct:["nationality"] }).then(r => r.map((x:any) => x.nationality)).catch(() => []);
  return (
    <main className="page">
