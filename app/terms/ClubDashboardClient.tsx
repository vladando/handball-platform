...
export default function ClubDashboardClient({ club, stats }: { club: any; stats: any }) {
  ...
  async function openCheckout() {
    setSubLoading(true);
    try {
      const res = await fetch("/api/club/checkout", { method: "POST" });
      const data = await res.json();
      if (!res.ok) { alert(data.error); setSubLoading(false); return; }

      const Paddle = (window as any).Paddle;
      if (!Paddle) { alert("Payment system not loaded. Please refresh the page."); setSubLoading(false); return; }

      Paddle.Checkout.open({
        items: [{ priceId: data.priceId, quantity: 1 }],
        customer: { email: data.customerEmail },
        customData: { clubId: data.clubId },
        settings: {
          theme: "dark",
          locale: "en",
          displayMode: "overlay",
        ...
        successUrl: `${window.location.origin}/dashboard/club?subscribed=1`,
      });
    } catch (e) {
      console.error(e);
    }
  ...
  // Load Paddle script dynamically after mount, then initialize
  useEffect(() => {
    function initPaddle() {
      const Paddle = (window as any).Paddle;
      if (!Paddle) return;
          ...
          {hasSubscription ? (
            ...
            <div className="card" style={{ marginBottom: 24, borderColor: "rgba(232,255,71,0.35)", background: "rgba(232,255,71,0.03)" }}>
              ...
              <button
                ...
                disabled={subLoading || !paddleReady}
                onClick={openCheckout}
              >
                {subLoading ? <><span className="spinner" /> Loading…</> : "💳 Subscribe Now — €1,000/year"}
              </button>
            </div>
          ) : (
