...
const POSITION_LABELS: Record<string, string> = {
  ...
  GK: "Goalkeeper", LW: "Left Wing", RW: "Right Wing",
  LB: "Left Back", RB: "Right Back", CB: "Centre Back", PV: "Pivot",
};
...
export default function PlayerDossierClient({ player, contracts, commissions, transfers, agentNotes, generalNotes }: Props) {
    ...
    <div style={{ minHeight: "100vh", background: "var(--black)", color: "var(--white)", fontFamily: "var(--font-body)" }}>
      ...
      <div style={{ borderBottom: "1px solid var(--border)", padding: "14px 24px", display: "flex", alignItems: "center", gap: 16, background: "var(--card)", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          <Link href={`/dashboard/agent/player/${player.id}/edit`} className="btn btn-outline" style={{ fontSize: "0.72rem", padding: "6px 14px" }}>✏️ Edit</Link>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 16px" }}>
        {/* Health banner */}
        {player.healthStatus && player.healthStatus !== "HEALTHY" && (
            ...
            <div className="card" style={{ padding: 0, overflow: "hidden" }}>
              ...
              <div style={{ padding: "16px 20px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {[
                  ...
                  ["Age", player.dateOfBirth ? `${getAge(player.dateOfBirth)} yrs` : "—"],
              ...
              <div key={e.id} className="card">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
                  <div>
                    ...
                    {e.country && <div style={{ fontSize: "0.75rem", color: "var(--muted)", marginTop: 2 }}>{e.country}{e.city ? `, ${e.city}` : ""}</div>}
                    {e.appearances != null && (
                      <div style={{ fontSize: "0.75rem", color: "var(--muted)", marginTop: 4 }}>
                        {e.appearances} apps{e.goals != null ? ` · ${e.goals} goals` : ""}{e.assists != null ? ` · ${e.assists} assists` : ""}
                      </div>
                    )}
                  </div>
                  <div style={{ textAlign: "right", fontFamily: "var(--font-mono)", fontSize: "0.78rem", color: "var(--muted)" }}>
                    ...
                    {e.isCurrentClub ? " — Present" : e.endDate ? ` — ${new Date(e.endDate).getFullYear()}` : ""}
                  </div>
                </div>
              </div>
            ))}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {player.videos.map((v: any) => (
                    <div key={v.id} className="card" style={{ display: "flex", gap: 12, alignItems: "center" }}>
                      ...
                      <a href={v.youtubeUrl} target="_blank" rel="noopener noreferrer" className="btn btn-outline" style={{ fontSize: "0.68rem", padding: "4px 10px" }}>▶ Watch</a>
                    </div>
                  ))}
                </div>
              </div>
